...
int Read (uint32_t Address, uint32_t Size, uint8_t* buffer)
{
	...
	spi_nor_read(Address, buffer, Size);
	LOC_LedOff();
    __set_PRIMASK(1); //disable interrupts
	return 1;
} 
...
int Write (uint32_t Address, uint32_t Size, uint8_t* buffer)
{
	...
	spi_nor_write(Address, buffer, Size);
	LOC_LedOff();
    __set_PRIMASK(1); //disable interrupts
	return 1;
} 
...
int SectorErase (uint32_t EraseStartAddress ,uint32_t EraseEndAddress)
{
    ...
    __set_PRIMASK(0); //enable interrupts
	EraseStartAddress = (EraseStartAddress -  (EraseStartAddress % SPI_NOR_SECTOR_SIZE));
	while (EraseEndAddress>=EraseStartAddress) {
		LOC_LedOn();
		//Flash_BErase64k(EraseStartAddress);
		spi_nor_block_erase_64k(EraseStartAddress);
		LOC_LedOff();
		EraseStartAddress += SPI_NOR_SECTOR_SIZE;
	}
    __set_PRIMASK(1); //disable interrupts
	return 1;
}
...
uint32_t CheckSum(uint32_t StartAddress, uint32_t Size, uint32_t InitVal)
{
	...
	for(cnt=0; cnt<Size ; cnt+=4)
	{
		...
		spi_nor_read(StartAddress + 1,(uint8_t *) &Val, 4);
		LOC_LedOff();
...
#define BUF_SIZE 2
	...
	{
		LOC_LedOn();
		spi_nor_read(MemoryAddr+VerifiedData, Buffer, BUF_SIZE);
		LOC_LedOff();
